...
void init( void ) {
    GLfloat xpos[] = {0,-0.6,0.6, 0.3, -0.3};
    GLfloat ypos[] = {-0.7,-0.55, -0.35, 0.2, -0.2};
    GLfloat zpos[] = {-5.5, -5.5, -4.5, -5.6, -4.1};
    //double size[2] = {0.8,0.8};
//    GLfloat ypos[3] = { 0.4, -0.5, 0.6};
//    GLfloat zpos[3] = { 0.3, -0.5, 0.6};
    for(int i =0;i<5;i++){
        Ball ball(xpos[i],ypos[i],zpos[i], i);
...
void keyboard( unsigned char key, int x, int y ) {
    ...
    if(key == 'w') ::ballVec[0].y+=0.25;
    

    if(key == 'a')  ::ballVec[0].x-=0.25;
    if(key == 's') ::ballVec[0].y-=0.25;
    if(key == 'd') ::ballVec[0].x+=0.25;
   // if(key == 'z') ::z-=0.25;
    if(key == 32){
        createKey();
    }
    ...
    if(key == 'p'){
        for(int i=0;i<ballVec.size();i++){
            std::cout << "ball" << ballVec[i].index << " is at: " << ballVec[i].x << ", " << ballVec[i].y << ", " << ballVec[i].z << std::endl;
        }
    }
    if(key == 't'){
        ::t += 0.05;
...
void specialKeyboard(int key, int x, int y){
    if(key == GLUT_KEY_UP) ::ballVec[0].z+=0.25;
    if(key == GLUT_KEY_DOWN) ::ballVec[0].z-=0.25;
